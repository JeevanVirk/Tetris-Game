...
class Game:
    ...
    def run(self):
        ...
        while True:
            ...
            if self.score % 50 and self.score != 0:
                # Spawn the Freeze power-up Tetromino
                self.tetromino = FreezePowerupFactory().create_power_tetromino(self.tetromino)
                self.update(command)
                pygame.time.set_timer(event_every_200ms, 1000)
